...
def test_cassage_brutal():
    ...
    texte_chiffre = double_chiffrement_sdes(CONST.PREMIERE_CLE_TEST,
                                            CONST.DEUXIEME_CLE_TEST,
                                            CONST.TEXTE_TEST)
    premiere_cle_trouvee, deuxieme_cle_trouvee = cassage_brutal(
        CONST.TEXTE_TEST, texte_chiffre)
    if premiere_cle_trouvee is not None and deuxieme_cle_trouvee is not None:
        ...
        texte_dechiffre_1 = dechiffrement_sdes(
            int(bin(premiere_cle_trouvee), 2), texte_chiffre)
        texte_dechiffre_2 = dechiffrement_sdes(
            int(bin(deuxieme_cle_trouvee), 2), texte_dechiffre_1)
...
def test_cassage_astucieux():
    ...
    texte_chiffre = double_chiffrement_sdes(CONST.PREMIERE_CLE_TEST,
                                            CONST.DEUXIEME_CLE_TEST,
                                            CONST.TEXTE_TEST)
    premiere_cle_trouvee, deuxieme_cle_trouvee = cassage_astucieux(
        CONST.TEXTE_TEST, texte_chiffre)
    if premiere_cle_trouvee is not None and deuxieme_cle_trouvee is not None:
        print("Clé 1:", bin(premiere_cle_trouvee))
        print("Clé 2:", bin(deuxieme_cle_trouvee))
